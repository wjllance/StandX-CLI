...
an opt-in WebSocket transport for single-order create/cancel.

### Added
- **Configurable StandX endpoint.** `--endpoint <url>` (global flag), `STANDX_BASE_URL`, and `standx config set base_url <url>` now determine which REST/WS host every command talks to — CLI flag, then env var, then a confirmed `config.toml`, then the production default. Previously `base_url` was stored in `config.toml` and shown by `config get`, but nothing that made a real request ever read it; every command used the hardcoded production endpoint regardless of what was configured. Plain `http://`/`ws://` is accepted only for `localhost` or a loopback address, for pointing at a local mock server during development — anything else must be `https://`
- **`--transport ws` for `order new` / `order cancel`.** Opt-in WebSocket path for placing or cancelling a single order (`cancel-all` stays REST-only); HTTP remains the default for both. The acknowledgement is correlated to the request by `request_id`, `--timeout-secs <1-30>` (default 10) bounds the wait, and a submission whose outcome can't be determined within that window is reported as unknown rather than silently falling back to a guess. `-v`/`--verbose` prints the raw inbound frame to stderr
...
- **Maker cleanup decides "residual" per order, not from the open-orders list.** After `cancel_orders`, each maker order is polled through `/api/query_order` until it reaches a terminal status (`filled` / `canceled` / `rejected` / `expired`) — 500ms, then 1s intervals, up to 6 attempts. `/api/query_open_orders` can lag ~15s behind a successful cancel, and on 2026-07-29 that lag alone truncated a 35.9h baseline run with a false fail-safe; per-order status is not subject to it. A non-terminal status is still residual and still fails closed, and the `maker_cleanup` JSON event now carries each order's observed `status` and `updated_at`
  ...
  - Cleanup's primary verdict is now the WS `order:cancel` acknowledgement itself when the order-response stream is healthy: it sends the cancel over WS, correlates the reply by `request_id`, and treats `is_success()` inside a 2s window as done — no `/api/query_order` round trip needed. It falls back to the REST path above on a send failure, a closed channel, a non-success ack, or an ack timeout; startup, shutdown, and reconnect explicitly stay REST-only since they have no healthy WS stream to use. The `maker_cleanup` event's per-order `confirmed_by` field now says which path decided (`ws_success` / `query_order`)
...
### Fixed
- **Maker no longer applies a rejection for an order the account stream has already put in the book.** Found by adversarial review of the request-correlation typed verdict (#277): if the account stream's `OrderObserved(open)` adopted a pending placement into the maker book before the command channel's own acknowledgement for that request arrived, and that acknowledgement then turned out to be a *rejection*, the old code applied the rejection as an ordinary `PlaceRejected` — unfreezing and skipping cleanup — while the order the account stream had just adopted stayed live and unmanaged. Worse, that adoption can match on side/price/quantity heuristics rather than an exact client-order-id, so the order the maker kept quoting around wasn't necessarily its own. A new `venue_observed` signal (set only on that stream adoption, cleared once cleanup revokes the exposure) now distinguishes this live two-channel disagreement — a new `VenueContradiction` verdict — from an ordinary late/duplicate ack: the rejection is not applied, the order stays in the book, and the run freezes for recovery to reconcile instead of trusting whichever channel answered first
- **Endpoint routing is hardened against silent misdirection and credential leakage**, now that it actually does something (see Added):
  - Upgrading no longer silently activates a stale `base_url`. It was cosmetic before this release, so some installs may carry a leftover custom value from earlier testing or a copied example. A `base_url` written before this version deserializes as unconfirmed and is ignored — falling back to the production endpoint, with a warning naming the file — until re-confirmed via `standx config set base_url <value>`
  - Every command except `market` (the only one that never loads a credential) now requires `STANDX_ALLOW_INSECURE_ENDPOINT=1` before it will send the JWT or signing key to a plain `http://`/`ws://` endpoint, closing a path where a mistyped `--endpoint` or a `STANDX_BASE_URL` picked up from a compromised script could leak a production credential to an unintended local listener in clear text
  - Known gap: the opt-in check lives in the CLI's own command dispatch, not in `standx-sdk` itself — a program embedding the SDK directly gets the same loopback-only HTTP restriction but not this second gate
- **Maker freeze cleanup's WS cancel verdict no longer fail-closes on its own acks, or silently drops one.** Found in review of the WS `order:cancel` primary-verdict path (docs/29 Phase 2) before merge, all three found together:
  - Cleanup only tombstoned a WS cancel's request ID when its ack *timed out*. The venue can answer one `order:cancel` with a gateway `accepted` frame and then the terminal `success` frame; since the drain releases an ID on its first frame, an untombstoned second frame classified as an unknown request ID and fail-closed the maker on a cancel it minted itself. Every request ID cleanup puts on the wire is now tombstoned (a bounded `CleanupTombstones` FIFO, capacity 512), and lookup no longer consumes the tombstone — both frames of a two-frame ack can now arrive after the drain window closes without either one tripping fail-closed
  - An order-response frame the cleanup drain captured for a pre-freeze cycle request (not one it minted) could be lost if a later step in the same cleanup attempt errored, because it was only returned via the function's return value — an early `?` return dropped it after it was already taken off the channel. That request's pending lifecycle then never resolved. The frame is now appended to a caller-owned buffer as soon as the drain captures it, so it survives any later failure in the same attempt
  - A leftover frame re-applied after cleanup that turned out to fail closed (e.g. it contradicted the local view) was downgraded to a bare health flag instead of stopping the run: the runtime is already frozen by the time leftovers replay, so the event the failure raises internally is a no-op, and the cleanup preamble went on to report success. That failure now routes through the same `CleanupFailed` stop a cleanup failure already uses
...
### Added
- **Maker stage 5-b (safety track, tier 2): graded exit policy and residual handoff** — the code-level prerequisite for scaling up. See `docs/archive/2026-07-maker/26-maker-stage5b-design.md`
