...
### Fixed
- **Maker freeze cleanup's WS cancel verdict no longer fail-closes on its own acks, or silently drops one.** Found in review of the WS `order:cancel` primary-verdict path (docs/29 Phase 2) before merge, all three still unreleased:
