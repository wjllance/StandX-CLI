...
- **性质定位**：`[external_skew]` 是**改善价格**的机制（每笔成交价格好一点），不是
  **避开成交**的机制（阻止坏成交发生）。尾部要的是后者，应作为独立候选（候选 6）立项。
- **⚠️ 与候选 6 的关系（2026-08-02 修正）**：原写"两者不互斥、不重叠"，**机制上不重叠成立，
  但资源上竞争**——候选 6 的落地形态取"加宽"而非"压侧"（压侧会踩 uptime guardrail，
  见尾部分解报告），而加宽要吃 band 预算。现状 `spread 8 + nonlinear cap 12 = 20`，余量
  10bps；**本机制 `cap_bps=8` 一开，余量只剩 2bps，候选 6 的加宽路线即不可行**。
  **两个候选可能不能简单地都开，裁决时须一并权衡。**
- **底线（2026-08-02）**：良性 90% 的单笔经济性本身为负
  （`capture 2.8 − markout 3.2 − 手续费 1 ≈ -1.4bps`）。本机制作用于这 90% 约
  +1.1bps/笔，把 -1.4 拉到 **-0.3**——接近但仍为负。**候选 4 + 候选 6 + maker 返佣
  （当前 `rebate=0`，1bps 量级）三者齐备才有可能翻正**；缺了返佣，另两个做满仍差最后一口。
  **⚠️ 2026-08-02 第三批更新：候选 6 主支已判死**（条件冷却的快代理够不着，减掉"等量随机
  压单"基线后信息价值 ≈ 净亏的 1.2%），候选 6 只剩 `post_suppressed` 一支、上限降到
  5~8%。**这条翻正路径因此断了一截，本机制成了三者中最大的那一个。** 由此浮出的问题
  （`capture 2.8bps` 能否养住 `1bps` 手续费 + `8bps` 逆选）已记入
  [尾部分解报告](evidence/maker-markout-tail-decomposition-2026-08-01.md)"大账更新"，
  属裁决范围，本文档不下判断。
