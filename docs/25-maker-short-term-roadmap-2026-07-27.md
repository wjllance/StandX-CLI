...
### 候选 6（原假设已证伪、形态已修订，设计待写）：尾部避让（有毒成交识别）

**这是目前最大的靶子**：60~72% 的 markout 亏损集中在最差 10% 的成交上（mo300
-53~-58bps），其余 90% 只有 -2.5~-4.0bps。成因是**场内**的（扫单 / 本所逆选）——外部
领先价对它零预警，故候选 4 结构上碰不到它。
...
| 支 | 覆盖 | 检测 | 状态 |
|---|---|---|---|
| **条件触发的成交后抑制**（主支） | ~48% 的有毒成交，只需动 ~10% 的成交 | **卡在快代理**（mo300 要 5min，live 只能用 mo30 / 即时 drift） | 待代理验证 |
| **`post_suppressed` 重进保护**（次支） | ~10% 的有毒成交 | **无需代理**（库存帽状态是自有状态，零延迟零误判） | 可直接 replay |
| `refresh_bps` 调整 | — | — | **撤，证据不足** |

**施工顺序：次支先落地。** 主支覆盖大但卡在检测；次支覆盖小却工程确定、验证便宜。
两者可并行（次支 replay 与主支代理研究互不阻塞）。
...
压到 2bps，**候选 6 的加宽路线即不可行**。两者机制不重叠但**资源竞争，可能不能都开**，
裁决须一并权衡。

**价值上限（修订）**：主支 ~48% + 次支 ~10% ≈ 50% 是**乐观相加**（两支同属趋势延续
regime，重叠待查）；且快代理有命中率损失、replay 前不知道躲开的成交会不会换个方式亏回来。
**规划口径维持 ≈ 净亏的 30%（候选 4 的两倍量级）**，先知上限约 45%。

**底线：两个候选都做满仍然在亏。** 现状 -1.66 → 候选 6 先知 50% ≈ -0.9 → 再加候选 4
≈ -0.64；即使整条尾部清零也只到 -0.35。根因是一个被尾部掩盖的结构性事实：**良性 90%
自身就不赚钱**（`capture 2.8 − markout 3.2 − 手续费 1 ≈ -1.4bps/笔`），所以尾部治理
不可能把它救成正的。

**第三个杠杆（唯一免费的）**：上述缺口里**手续费占 1bps**，而冻结基线 `rebate = 0`。
若存在 maker 返佣档位，这 1bps 直接把候选 4 之后的 -0.3bps 变成 +0.7bps，**不改策略、
不占 live 时间片、不需要 A/B**。**候选 4 + 候选 6 + 返佣三者齐备才有可能翻正。**
行动项：确认是否有 maker 返佣档位、以及 SIP-5A 在什么规模开始有实际返还。

**下一批查询（均免费，按重要性）**：

1. **代理的决策口径验证**（口径已改）：不验"代理与 mo300 标签的吻合度"，直接验
   **P(下一笔同侧有毒 | 代理在本笔触发)** 对照基础率 9.9%，**并同时记触发频率**
   （触发频率是 uptime 预算的硬约束，不是附带指标）。候选代理：mo30 阈值、即时 drift、
   mo5、成交价对 mark 的偏离。
2. **有毒成交间隔的 p75 / p90** —— 窗长由它定，而窗长受 uptime 红线约束。
3. **两支的重叠交叉表** —— 覆盖率能否相加。
4. **同价重复挂单的稳健性重跑** —— 归因歧义不只影响年龄，也影响"出生方式"标签，而表 4/5
   都建在该标签上。

**排序建议**：live 时间片稀缺，A/B 按当前亏损速率（-0.046 DUSD/h）跑 140h 约烧
6.5 DUSD ≈ 权益 3.5%，**超过单次 `stop_loss=5.0` 预算**。候选 6 上限仍高于候选 4 且离线
查询免费，故不宜把 live 时间片先花在候选 4 上。**08-02 的教训**：连"抬高 `refresh_bps`"
