...
**待验证的成因假设（不占 live 时间片，是本候选立项的前置）**：是否"我们自己撞上去"
——refresh（漂移 > `refresh_bps=4`）触发撤单重挂，新单落进仍在移动的市场后立刻被扫。
验证只需现有 place / cancel / fill 时间线：那 103 笔是否多由 refresh 重挂产生、成交前
漂移方向与成交后移动方向是否同向。

**这一步的答案决定机制形态**，所以设计文档尚未开写：

- **成立** → 对策是**重挂纪律**（大幅移动后延迟补挂 / 临时加宽），可能是配置量级的
  改动，**不需要 140h 的 A/B**；
- **不成立** → 对策是**新单保护 / 打穿即撤**一类新机制，按 18 号 v0 流程正式立项。

**排序建议**：live 时间片是稀缺资源，而 A/B 按当前亏损速率（-0.046 DUSD/h）跑 140h
约烧 6.5 DUSD ≈ 权益 3.5%，**超过单次运行的 `stop_loss=5.0` 预算**。因此不宜把 live
时间片先花在 16% 的杠杆上，而 72% 的杠杆还差一次免费的离线查询就能有设计。
