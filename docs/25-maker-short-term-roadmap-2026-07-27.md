...
- **上限更明确**：亏损 **60~72% 集中在最差 10% 的成交**（mo300 -53~-58bps），而外部
  ...
  ——裁决不能按"这一个就能救活"来批。
- **性质**：这是**改善价格**的机制，不是**避开成交**的机制。与下面的尾部候选
  不互斥、不重叠。
...
**修订后的机制形态（两支并存，数据不支持二选一）**：

- **成交后补挂纪律**（主支）：同侧成交后延迟 / 加宽补挂，触发条件挂在那笔成交自身的
  不利特征上（90% 的成交是好的，无条件冷却会白扔 uptime 与 capture）。依据：有毒非 rb
  单距上一笔同侧成交中位 138s / <30s 占 32%，对照其余组 226s / 22%。
  **只覆盖有毒单的约 1/4~1/3。**
- **新单保护 / 打穿即撤**（保留）：剩下约 2/3 的有毒单仍指向"单挂在原地被带信息的对手方
  扫走"，按 18 号 v0 流程立项。

**价值上限（修订，重要）**：每 36h 尾部 markout 约 1.51，主支若砍掉 1/3 的有毒单约
**+0.50 / 36h ≈ 净亏的 30%**——即候选 6 约值候选 4（~16%）的**两倍，不是四倍**。
**60~72% 的尾部不等于可达范围；两个候选都做满仍不到盈亏平衡**（整条尾部全砍，剩余
90% 每笔仍差约 1.4bps）。

**待验证的新假设：成交扎堆（大单分批执行）**——有毒单集中在补挂那批 + 有毒组
`drift_place` 对 mo30 有解释力（r≈0.27 vs 0.08）+ mo30 的 deff 1.8→3.1 且 4h 未封顶，
三条可以是同一件事：大单扫掉我方一次、我方补挂、它继续扫。判死检验 = 同侧成交序列中
毒性的自相关。

**下三个查询（均不占 live 时间片，决定设计能否开写）**：四格单龄分布（拆"年龄"vs
"出生方式"）；有毒非 rb 单按**前驱离场原因**归因（比时间代理锐利，gap 口径改为"前一笔
成交 → 本单挂出"）；同侧成交毒性自相关。

**排序建议**：live 时间片是稀缺资源，而 A/B 按当前亏损速率（-0.046 DUSD/h）跑 140h
约烧 6.5 DUSD ≈ 权益 3.5%，**超过单次运行的 `stop_loss=5.0` 预算**。候选 6 的上限
（~30%）仍高于候选 4（~16%）且离线查询免费，因此不宜把 live 时间片先花在候选 4 上。
**但注意 08-02 的教训**：连"抬高 `refresh_bps`"这种看着无害的配置旋钮都可能方向搞反，
所以候选 6 即使落到纯配置形态，也必须先过离线 replay（`maker_creep_replay.py`）确认
不是把一种毒性换成另一种，再谈 live。
