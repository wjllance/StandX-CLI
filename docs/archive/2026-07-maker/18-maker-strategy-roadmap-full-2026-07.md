...
> **归档记录**：这是 2026-07 阶段路线的完整历史快照，包含已经完成、被拒或被后续证据
> 取代的计划。它不能作为当前实现、操作流程或 live 授权依据；当前状态见
...
每一轮实验的立项、判据入库、启动记录与判定报告的通用规程见
[28-experiment-protocol.md](../../28-experiment-protocol.md)（含预注册判据模板与未判项登记表）。
